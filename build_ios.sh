...
#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_ios.sh — console-only iOS build for AoSChat.
#
# Produces an UNSIGNED  build-ios/AoSChat.app . Signing + install to a device
...
# extracted bsresources.zip, overrides) into <root>/build/AoSChat via its
# RUNTIME_OUTPUT_DIRECTORY + POST_BUILD steps. We fold that into the bundle and
# drop in the Info.plist.
PAYLOAD="$ROOT/build/AoSChat"
APP="$BUILD/AoSChat.app"
