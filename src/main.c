...
void mouse_scroll(struct window_instance* window, double xoffset, double yoffset) {
        /* A HUD-specific callback owns scrolling when present. Sending the same
           wheel event to microui as well made both the chat panel and its root
           window move, eventually corrupting the layout. */
...
static void ios_stage_resources_and_chdir(void) {
        ...
        char* pref = SDL_GetPrefPath("AoS Chat", "client"); /* writable sandbox */
        if(!pref) {
        ...
        if(sf) {
                ...
                if(fgets(have, sizeof(have), sf))
                        need_stage = strcmp(have, AOSCHAT_VERSION) != 0;
                fclose(sf);
        }
        ...
        if(need_stage && base) {
                ...
                sf = fopen(stamp, "w");
                if(sf) { fputs(AOSCHAT_VERSION, sf); fclose(sf); }
        }
...
int main(int argc, char** argv) {
        ...
        settings.fullscreen = 0;
        settings.android_lock_portrait = 1;
        ...
        settings.spectator_speed = 1.0F;
        settings.spectator_acceleration = 80.0F;
        settings.spectator_fog_distance = 128.0F;
        ...
        strcpy(settings.name, "AoSChatUser");
...
#elif defined(OS_APPLE)
        {
                ...
                char* pref = SDL_GetPrefPath("AoS Chat", "client");
                if(pref) {
                        if(base) {
                                char res[2048];
