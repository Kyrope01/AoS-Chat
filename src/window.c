...
void window_title(char* suffix) {
	if(suffix) {
		char title[128];
		snprintf(title, sizeof(title) - 1, "AoS Chat %s - %s", GIT_COMMIT_HASH, suffix);
		glfwSetWindowTitle(hud_window->impl, title);
	} else {
		glfwSetWindowTitle(hud_window->impl, "AoS Chat " GIT_COMMIT_HASH);
	}
}
...
void window_init() {
	...
	*/
	...
	hud_window->impl
		= glfwCreateWindow(settings.window_width, settings.window_height, "AoS Chat " AOSCHAT_VERSION,
...
#ifdef __ANDROID__
/* Hide/show only the top status bar via AoSChatActivity.setStatusBarHidden,
...
void window_init() {
	...
	/* AoS Chat is portrait-first on Android. When the lock is disabled, expose
...
retry_context:
	...
	hud_window->impl
		= SDL_CreateWindow("AoS Chat " AOSCHAT_VERSION, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
		...
		switch(event.type) {
			...
			case SDL_FINGERMOTION: {
				...
				if(f && f->dragged && !holding_widget) {
					/* As with desktop wheel input, a HUD callback takes exclusive
					   ownership. This makes touch-hold-and-drag scroll only the
					   fullscreen chat history, never the root window. */
					if(hud_active->input_mousescroll)
						hud_active->input_mousescroll(fdy / 50.0F);
					else if(hud_active->ctx)
...
void window_title(char* suffix) {
	if(suffix) {
		char title[128];
		snprintf(title, sizeof(title) - 1, "AoS Chat %s - %s", AOSCHAT_VERSION, suffix);
		SDL_SetWindowTitle(hud_window->impl, title);
	} else {
		SDL_SetWindowTitle(hud_window->impl, "AoS Chat " AOSCHAT_VERSION);
	}
}
