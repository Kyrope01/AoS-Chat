...
void config_save() {
        ...
        config_seti("client", "android_lock_portrait", settings.android_lock_portrait);
...
static int config_read_key(void* user, const char* section, const char* name, const char* value) {
        if(!strcmp(section, "client")) {
                ...
                IMPORT_SETTING(settings.android_lock_portrait, android_lock_portrait, atoi(value) ? 1 : 0);
...
void config_reload() {
        ...
        list_add(&config_settings,
                         &(struct config_setting) {
                                 .value = &settings_tmp.android_lock_portrait,
                                 .type = CONFIG_TYPE_INT,
                                 .min = 0,
                                 .max = 1,
                                 .name = "Lock to portrait mode",
                                 .help = "Lock portrait; disable to follow device rotation",
                         });
#endif
        list_add(&config_settings,
                         &(struct config_setting) {
        ...
        /* AoS Chat never runs the post-processing pipeline. Existing client
           config files may contain enabled effects, so force neutral values
           after parsing rather than merely hiding their controls. */
        settings.exposure = 0.0F;
        settings.saturation = 0.0F;
        settings.contrast = 0.0F;
        settings.vignette = 0.0F;
        settings.volumetric_light = 0;
        settings.lens_flare = 0;
