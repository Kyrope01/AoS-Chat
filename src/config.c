...
void config_save() {
	...
	config_seti("client", "dust_storm", settings.dust_storm);
...
static int config_read_key(void* user, const char* section, const char* name, const char* value) {
	if(!strcmp(section, "client")) {
		...
		IMPORT_SETTING(settings.dust_storm, dust_storm, atoi(value));
...
void config_reload() {
	...
	list_add(&config_settings,
			 &(struct config_setting) {
				 .value = &settings_tmp.dust_storm,
				 .type = CONFIG_TYPE_INT,
				 .min = 0,
				 .max = 1,
				 .help = "Enable dust storm weather effect",
				 .name = "Dust Storm",
				 .category = "Weather",
			 });
