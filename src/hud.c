...
/*
        ...
        This file is part of AoSChat.

        AoSChat is free software: you can redistribute it and/or modify
        it under the terms of the GNU General Public License as published by
        the Free Software Foundation, either version 3 of the License, or
        (at your option) any later version.

        AoSChat is distributed in the hope that it will be useful,
        but WITHOUT ANY WARRANTY; without even the implied warranty of
        MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
        GNU General Public License for more details.

        You should have received a copy of the GNU General Public License
        along with AoSChat.  If not, see <http://www.gnu.org/licenses/>.
*/
...
/* AoS Chat application state. The existing engine remains underneath for
   protocol compatibility and spectator rendering, but normal team/gameplay
   entry points are never exposed. */
static int aoschat_spectator_active = 0;
static int aoschat_spectator_menu = 0;
...
static mu_Rect aoschat_spec_chat_rect;
static mu_Rect aoschat_spec_fullchat_rect;
static mu_Rect aoschat_spec_players_rect;
static mu_Rect aoschat_spec_leave_rect;
static mu_Rect aoschat_spec_rows[PLAYERS_MAX];
static int aoschat_spec_row_ids[PLAYERS_MAX];
static int aoschat_spec_row_count = 0;

static void aoschat_spectator_render_2d(float scalef);
static int aoschat_spectator_keyboard(int key, int action, int mods, int internal);
static int aoschat_spectator_mouseclick(double x, double y, int button, int action, int mods);
static int aoschat_hit(mu_Rect r, double x, double y);
...
void hud_init() {
        ...
        hud_aoschat.ctx = malloc(sizeof(mu_Context));
...
#ifdef JENKINS_BUILD
        if(show_update_popup && mu_begin_window_ex(ctx, "New AoSChat version available", mu_rect(0, 0, 350, 155),
...
static void hud_ingame_render(mu_Context* ctx, float scalex, float scalef) {
        if(aoschat_spectator_active && !network_map_transfer) {
                aoschat_spectator_render_2d(scalef);
                return;
        }
...
void hud_ingame_mouseclick(double x, double y, int button, int action, int mods) {
        if(aoschat_spectator_active
           && aoschat_spectator_mouseclick(x, y, button, action, mods))
                return;
        if(chat_input_mode != CHAT_NO_INPUT) {
...
static void hud_ingame_keyboard(int key, int action, int mods, int internal) {
        if(aoschat_spectator_active
           && aoschat_spectator_keyboard(key, action, mods, internal))
                return;
...
static void hud_ingame_touch(void* finger, int action, float x, float y, float dx, float dy) {
        ...
        if(aoschat_spectator_active) {
                int over_ui = aoschat_hit(aoschat_spec_menu_rect, x, y)
                        || aoschat_hit(aoschat_spec_chat_rect, x, y)
                        || aoschat_hit(aoschat_spec_fullchat_rect, x, y)
                        || aoschat_hit(aoschat_spec_players_rect, x, y)
                        || aoschat_hit(aoschat_spec_leave_rect, x, y);
                for(int k = 0; !over_ui && k < aoschat_spec_row_count; k++)
                        over_ui = aoschat_hit(aoschat_spec_rows[k], x, y);
                if(action == TOUCH_DOWN && over_ui) {
                        aoschat_spectator_mouseclick(x, y, WINDOW_MOUSE_LMB, WINDOW_PRESS, 0);
                        return;
                }
                if(over_ui || aoschat_spectator_menu || aoschat_playerlist_open
                   || chat_input_mode != CHAT_NO_INPUT)
                        return;
                /* With all overlays closed, fall through to the proven mobile
                   spectator joystick/look implementation below. */
        }
...
/*         AOS CHAT APPLICATION HUD START        */

static int aoschat_full_playerlist = 0;
static int aoschat_settings_return_connected = 0;
static mu_Container* aoschat_messages_panel = NULL;

static void aoschat_plain_text(char* out, size_t out_size, const char* in) {
        size_t n = 0;
        if(!out_size) return;
        for(; *in && n + 1 < out_size; in++) {
                unsigned char c = (unsigned char)*in;
                if((c >= 1 && c <= 7) || c == 0xFF) continue;
                if(c >= 0x20 || c == '\t') out[n++] = (char)c;
        }
        out[n] = 0;
}

static int aoschat_split_player_message(const char* raw, char* prefix, size_t prefix_sz,
                                        char* name, size_t name_sz, char* body, size_t body_sz) {
        const char* color = NULL;
        const char* reset = NULL;
        for(const char* p = raw; *p; p++) {
                unsigned char c = (unsigned char)*p;
                if(!color && c >= 1 && c <= 3) color = p;
                else if(color && c == 6) { reset = p; break; }
        }
        if(!color || !reset || reset <= color + 1) {
                /* Some server/client paths sanitize inline color bytes before the
                   message reaches the live ring. Recover player structure from
                   the visible "[Global] Name: message" form so spectator chat
                   can still color the name from the current player table. */
                char plain[256];
                aoschat_plain_text(plain, sizeof(plain), raw);
                const char* name_start = plain;
                const char* visible_prefix = "";
                if(!strncmp(name_start, "[Global] ", 9)) {
                        visible_prefix = "[Global] ";
                        name_start += 9;
                }
                const char* colon = strchr(name_start, ':');
                if(colon) {
                        size_t candidate_len = (size_t)(colon - name_start);
                        while(candidate_len > 0 && name_start[candidate_len - 1] == ' ')
                                candidate_len--;
                        int matched = 0;
                        for(int i = 0; i < PLAYERS_MAX; i++) {
                                if(players[i].connected
                                   && strlen(players[i].name) == candidate_len
                                   && !strncmp(players[i].name, name_start, candidate_len)) {
                                        matched = 1;
                                        break;
                                }
                        }
                        if(matched) {
                                strncpy(prefix, visible_prefix, prefix_sz - 1);
                                prefix[prefix_sz - 1] = 0;
                                size_t nn = min(name_sz - 1, candidate_len);
                                memcpy(name, name_start, nn); name[nn] = 0;
                                const char* message_start = colon + 1;
                                while(*message_start == ' ') message_start++;
                                strncpy(body, message_start, body_sz - 1);
                                body[body_sz - 1] = 0;
                                return 1;
                        }
                }
                prefix[0] = name[0] = 0;
                strncpy(body, plain, body_sz - 1);
                body[body_sz - 1] = 0;
                return 0;
        }
        size_t pn = min(prefix_sz - 1, (size_t)(color - raw));
        memcpy(prefix, raw, pn); prefix[pn] = 0;
        size_t nn = min(name_sz - 1, (size_t)(reset - color - 1));
        memcpy(name, color + 1, nn); name[nn] = 0;
        aoschat_plain_text(body, body_sz, reset + 1);
        char* message_start = body;
        if(*message_start == ':') message_start++;
        while(*message_start == ' ') message_start++;
        if(message_start != body) memmove(body, message_start, strlen(message_start) + 1);
        return 1;
}

static unsigned int aoschat_player_name_color(const char* name, unsigned int fallback) {
        for(int i = 0; i < PLAYERS_MAX; i++) {
                if(players[i].connected && !strcmp(players[i].name, name)) {
                        if(players[i].team == TEAM_1)
                                return rgb((int)(gamestate.team_1.red * 0.70F + 76),
                                           (int)(gamestate.team_1.green * 0.70F + 76),
                                           (int)(gamestate.team_1.blue * 0.70F + 76));
                        if(players[i].team == TEAM_2)
                                return rgb((int)(gamestate.team_2.red * 0.70F + 76),
                                           (int)(gamestate.team_2.green * 0.70F + 76),
                                           (int)(gamestate.team_2.blue * 0.70F + 76));
                        return rgb(220, 220, 220);
                }
        }
        return fallback ? fallback : rgb(220, 220, 220);
}

struct aoschat_wrapped_line {
        int start;
        int length;
};

static int aoschat_utf8_next(const char* text, int pos, int length) {
        if(pos >= length) return length;
        pos++;
        while(pos < length && (((unsigned char)text[pos] & 0xC0) == 0x80)) pos++;
        return pos;
}

static int aoschat_wrap_lines(mu_Context* ctx, const char* text,
                              int first_width, int full_width,
                              struct aoschat_wrapped_line* lines, int max_lines) {
        int length = (int)strlen(text);
        int pos = 0, count = 0;
        while(pos < length && count < max_lines) {
                while(pos < length && text[pos] == ' ') pos++;
                if(pos >= length) break;
                int limit = count == 0 ? first_width : full_width;
                if(limit < 1) limit = 1;
                int cur = pos, previous = pos, last_space = -1;
                while(cur < length) {
                        int next = aoschat_utf8_next(text, cur, length);
                        if(text[cur] == ' ') last_space = cur;
                        if(ctx->text_width(ctx->style->font, text + pos, next - pos) > limit) {
                                int cut = last_space > pos ? last_space : previous;
                                if(cut <= pos) cut = next;
                                lines[count].start = pos;
                                lines[count].length = cut - pos;
                                pos = cut;
                                break;
                        }
                        previous = next;
...
static void aoschat_mu_outlined(mu_Context* ctx, const char* text, int len,
                                mu_Vec2 pos, mu_Color color) {
        mu_Color black = mu_color(0, 0, 0, 255);
        mu_draw_text(ctx, ctx->style->font, text, len, mu_vec2(pos.x - 1, pos.y), black);
        mu_draw_text(ctx, ctx->style->font, text, len, mu_vec2(pos.x + 1, pos.y), black);
        mu_draw_text(ctx, ctx->style->font, text, len, mu_vec2(pos.x, pos.y - 1), black);
        mu_draw_text(ctx, ctx->style->font, text, len, mu_vec2(pos.x, pos.y + 1), black);
...
static void aoschat_send_message(const char* text) {
        if(!text || !text[0] || !network_connected || !network_logged_in)
                return;
        struct PacketChatMessage msg;
        ...
        chat_add(2, 0, msg.message);
        sound_create(SOUND_LOCAL, &sound_chat, 0.0F, 0.0F, 0.0F);
}

static void aoschat_enter_spectator(int player_id) {
        aoschat_spectator_active = 1;
        aoschat_spectator_menu = 0;
        aoschat_playerlist_open = 0;
        camera_mode = CAMERAMODE_SPECTATOR;
        screen_current = SCREEN_NONE;
        cameracontroller_reset_spectator_velocity();
...
static void hud_aoschat_init(void) {
        aoschat_spectator_active = 0;
        aoschat_full_playerlist = 0;
        aoschat_selected_player = -1;
        aoschat_messages_panel = NULL;
        aoschat_input[0] = 0;
        chat_input_mode = CHAT_NO_INPUT;
...
static void aoschat_render_player_window(mu_Context* ctx) {
        if(!aoschat_full_playerlist) return;
        int w = min((int)(420 * hud_ui_scale()), settings.window_width * 9 / 10);
        int h = min((int)(520 * hud_ui_scale()), settings.window_height * 8 / 10);
        if(mu_begin_window_ex(ctx, "Online players",
                mu_rect((settings.window_width - w) / 2, (settings.window_height - h) / 2, w, h),
                MU_OPT_NORESIZE | MU_OPT_NOCLOSE)) {
                mu_Container* cnt = mu_get_current_container(ctx);
                cnt->rect = mu_rect((settings.window_width - w) / 2,
                                    (settings.window_height - h) / 2, w, h);
                /* The message panel is a full-screen root; explicitly keep this
                   modal above it instead of relying on creation-order z values. */
                mu_bring_to_front(ctx, cnt);
                mu_layout_row(ctx, 1, (int[]){-1}, 0);
                mu_text(ctx, "Tap/click a player twice to spectate");
                for(int i = 0; i < PLAYERS_MAX; i++) {
                        if(!players[i].connected || players[i].team == TEAM_SPECTATOR
                           || i == local_player_id) continue;
                        mu_push_id(ctx, &i, sizeof(i));
                        if(players[i].team == TEAM_1)
                                mu_text_color(ctx, gamestate.team_1.red, gamestate.team_1.green, gamestate.team_1.blue);
                        else if(players[i].team == TEAM_2)
                                mu_text_color(ctx, gamestate.team_2.red, gamestate.team_2.green, gamestate.team_2.blue);
                        else
                                mu_text_color_default(ctx);
                        if(mu_button(ctx, players[i].name)) {
                                if(aoschat_selected_player == i) {
                                        aoschat_full_playerlist = 0;
                                        aoschat_enter_spectator(i);
                                } else {
                                        aoschat_selected_player = i;
                                }
                        }
                        mu_text_color_default(ctx);
                        mu_pop_id(ctx);
                }
                if(mu_button(ctx, "Close")) aoschat_full_playerlist = 0;
                mu_end_window(ctx);
        }
}

static void hud_aoschat_render(mu_Context* ctx, float scalex, float scaley) {
        (void)scalex; (void)scaley;
        hud_common_render(ctx);
        mu_Rect frame = mu_rect(0, 0, settings.window_width, settings.window_height);
        if(mu_begin_window_ex(ctx, "AoS Chat", frame,
                             MU_OPT_NOFRAME | MU_OPT_NOTITLE | MU_OPT_NORESIZE | MU_OPT_NOSCROLL)) {
                mu_Container* root = mu_get_current_container(ctx);
                root->rect = frame;
                int bw = (int)(100 * hud_ui_scale());
                int gap = ctx->style->spacing;
                int portrait = settings.window_height > settings.window_width;
                if(portrait) {
                        mu_layout_row(ctx, 1, (int[]){-1}, 0);
                        mu_text(ctx, network_connected ? "Connected - spectator chat" : "Connecting...");
                        int third = max(40, (settings.window_width - gap * 4) / 3);
                        mu_layout_row(ctx, 3, (int[]){third, third, -1}, 0);
                } else {
                        int status_w = max(40, settings.window_width - bw * 3 - gap * 4);
                        mu_layout_row(ctx, 4, (int[]){status_w, bw, bw, bw}, 0);
                        mu_text(ctx, network_connected ? "Connected - spectator chat" : "Connecting...");
                }
                if(mu_button(ctx, "Players")) aoschat_full_playerlist = !aoschat_full_playerlist;
                if(mu_button(ctx, "Spectate")) aoschat_enter_spectator(-1);
                if(mu_button(ctx, "Settings")) {
                        aoschat_settings_return_connected = 1;
                        hud_change(&hud_settings);
                }
                ...
                aoschat_messages_panel = messages;
                static int last_message_count = -1;
                int visible_message_count = session_log_count;
                if(visible_message_count == 0)
                        for(int i = 1; i < 128; i++) visible_message_count += !!chat[0][i][0];
                int scroll_new_message_to_bottom = last_message_count != visible_message_count;
                last_message_count = visible_message_count;
                mu_end_panel(ctx);
                if(scroll_new_message_to_bottom) {
                        /* mu_end_panel has now finalized the new content height. */
                        messages->scroll.y = max(0, messages->content_size.y
        ...
        aoschat_render_player_window(ctx);
}

static void hud_aoschat_keyboard(int key, int action, int mods, int internal) {
        (void)mods; (void)internal;
        if(action != WINDOW_PRESS) return;
        if(key == WINDOW_KEY_ESCAPE) {
                if(aoschat_full_playerlist) aoschat_full_playerlist = 0;
...
static void hud_aoschat_touch(void* finger, int action, float x, float y, float dx, float dy) {
        (void)finger; (void)action; (void)dx; (void)dy;
        window_setmouseloc(x, y);
}

static void hud_aoschat_scroll(double yoffset) {
        if(!aoschat_messages_panel) return;
        int max_scroll = max(0, aoschat_messages_panel->content_size.y
                                + hud_aoschat.ctx->style->padding * 2
                                - aoschat_messages_panel->body.h);
        aoschat_messages_panel->scroll.y = max(0, min(max_scroll,
                aoschat_messages_panel->scroll.y - (int)(yoffset * 50.0)));
        /* The fullscreen root is deliberately fixed; never allow wheel/touch
           input to leak into and displace the whole application window. */
        if(hud_aoschat.ctx) {
                mu_Container* root = mu_get_container(hud_aoschat.ctx, "AoS Chat");
                if(root) root->scroll = mu_vec2(0, 0);
        }
}

struct hud hud_aoschat = {
        hud_aoschat_init, NULL, hud_aoschat_render, hud_aoschat_keyboard,
        NULL, NULL, hud_aoschat_scroll, hud_aoschat_touch, NULL,
        1, 0, NULL,
};

static void aoschat_draw_button_tinted(mu_Rect r, const char* label, int active,
                                        int text_r, int text_g, int text_b) {
        int ar, ag, ab;
        hud_accent_rgb(&ar, &ag, &ab);
        glColor4ub(active ? ar : ar / 2, active ? ag : ag / 2,
                   active ? ab : ab / 2, 220);
        glEnable(GL_BLEND);
        glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
        texture_draw_empty(r.x, settings.window_height - r.y, r.w, r.h);
        glDisable(GL_BLEND);
        glColor3ub(text_r, text_g, text_b);
        font_centered(r.x + r.w * 0.5F,
                      settings.window_height - r.y - r.h * 0.25F,
                      16.0F * hud_ui_scale(), (char*)label);
}

static void aoschat_draw_button(mu_Rect r, const char* label, int active) {
        aoschat_draw_button_tinted(r, label, active, 255, 255, 255);
}

static void aoschat_render_recent_messages(void) {
        int shown = 0;
        const float h = 16.0F;
        for(int i = 1; i < 128 && shown < 8; i++) {
                if(!chat[0][i][0]) continue;
                char prefix[32], name[32], body[256];
                int player_message = aoschat_split_player_message(chat[0][i],
                        prefix, sizeof(prefix), name, sizeof(name), body, sizeof(body));
                float y = 90.0F + shown * 20.0F;
                if(player_message) {
                        float total = font_length(h, prefix) + font_length(h, name)
                                + font_length(h, ": ") + font_length(h, body);
                        float x = settings.window_width * 0.5F - total * 0.5F;
                        glColor3ub(255, 255, 255);
                        hud_font_render_outlined(x, y, h, prefix, 1.0F);
                        x += font_length(h, prefix);
                        unsigned int c = aoschat_player_name_color(name, chat_color[0][i]);
                        glColor3ub(red(c), green(c), blue(c));
                        hud_font_render_outlined(x, y, h, name, 1.0F);
                        x += font_length(h, name);
                        glColor3ub(255, 255, 255);
                        hud_font_render_outlined(x, y, h, ": ", 1.0F);
                        x += font_length(h, ": ");
                        hud_font_render_outlined(x, y, h, body, 1.0F);
                } else {
                        glColor3ub(255, 255, 255);
                        float x = settings.window_width * 0.5F - font_length(h, body) * 0.5F;
                        hud_font_render_outlined(x, y, h, body, 1.0F);
                }
                shown++;
        }
...
static void aoschat_spectator_render_2d(float scalef) {
        (void)scalef;
        aoschat_render_recent_messages();
#ifdef USE_TOUCH
        /* Keep the proven spectator movement controls for phones. */
        glColor3f(1.0F, 1.0F, 1.0F);
        texture_draw_rotated(&texture_ui_joystick, settings.window_height * 0.3F,
                             settings.window_height * 0.3F, settings.window_height * 0.4F,
                             settings.window_height * 0.4F, 0.0F);
        texture_draw_rotated(&texture_ui_knob, hud_ingame_touch_x + settings.window_height * 0.3F,
        ...
        aoschat_spec_menu_rect = mu_rect(settings.window_width - bw - 12, 12, bw, bh);
        aoschat_draw_button(aoschat_spec_menu_rect, "Menu", aoschat_spectator_menu);

        aoschat_spec_chat_rect = aoschat_spec_fullchat_rect = aoschat_spec_players_rect
                = aoschat_spec_leave_rect = mu_rect(0,0,0,0);
        if(aoschat_spectator_menu) {
                aoschat_spec_chat_rect = mu_rect(aoschat_spec_menu_rect.x,
                                                 aoschat_spec_menu_rect.y + bh + 6, bw, bh);
                aoschat_spec_fullchat_rect = mu_rect(aoschat_spec_menu_rect.x,
                                                     aoschat_spec_chat_rect.y + bh + 6, bw, bh);
                aoschat_spec_players_rect = mu_rect(aoschat_spec_menu_rect.x,
                                                    aoschat_spec_fullchat_rect.y + bh + 6, bw, bh);
                aoschat_draw_button(aoschat_spec_chat_rect, "Quick chat", 1);
                aoschat_draw_button(aoschat_spec_fullchat_rect, "Full chat", 1);
                aoschat_draw_button(aoschat_spec_players_rect, "Player list", aoschat_playerlist_open);
        }
        if(cameracontroller_bodyview_mode) {
                aoschat_spec_leave_rect = mu_rect(12, 12, (int)(150 * ui), bh);
                aoschat_draw_button(aoschat_spec_leave_rect, "Leave player cam", 1);
        }

        if(chat_input_mode != CHAT_NO_INPUT) {
                int w = min((int)(560 * ui), settings.window_width - 24);
                mu_Rect box = mu_rect((settings.window_width - w) / 2,
                                      settings.window_height - (int)(56 * ui), w, (int)(42 * ui));
                glColor4ub(0, 0, 0, 190);
                glEnable(GL_BLEND); glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
                texture_draw_empty(box.x, settings.window_height - box.y, box.w, box.h);
                glDisable(GL_BLEND);
                glColor3ub(255,255,255);
                hud_font_render_outlined(box.x + 8,
                                         settings.window_height - box.y - box.h * 0.35F,
                                         16.0F * ui, chat[0][0], 1.0F);
        }

        aoschat_spec_row_count = 0;
        if(aoschat_playerlist_open) {
                int panel_w = min((int)(360 * ui), settings.window_width - 24);
                int row_h = (int)(30 * ui);
                int x = (settings.window_width - panel_w) / 2;
                int y = (int)(70 * ui);
                for(int i = 0; i < PLAYERS_MAX && aoschat_spec_row_count < PLAYERS_MAX; i++) {
                        if(!players[i].connected || players[i].team == TEAM_SPECTATOR
                           || i == local_player_id) continue;
                        mu_Rect row = mu_rect(x, y + aoschat_spec_row_count * row_h, panel_w, row_h - 2);
                        aoschat_spec_rows[aoschat_spec_row_count] = row;
                        aoschat_spec_row_ids[aoschat_spec_row_count] = i;
                        int tr = 220, tg = 220, tb = 220;
                        if(players[i].team == TEAM_1) {
                                tr = gamestate.team_1.red; tg = gamestate.team_1.green; tb = gamestate.team_1.blue;
                        } else if(players[i].team == TEAM_2) {
                                tr = gamestate.team_2.red; tg = gamestate.team_2.green; tb = gamestate.team_2.blue;
                        }
                        aoschat_draw_button_tinted(row, players[i].name,
                                                   aoschat_selected_player == i, tr, tg, tb);
                        aoschat_spec_row_count++;
                }
        }
}
...
static int aoschat_spectator_keyboard(int key, int action, int mods, int internal) {
        ...
        if(chat_input_mode != CHAT_NO_INPUT) {
                if(key == WINDOW_KEY_ENTER || key == WINDOW_KEY_ESCAPE) {
                        if(key == WINDOW_KEY_ENTER) aoschat_send_message(chat[0][0]);
                        chat[0][0][0] = 0;
                        chat_input_mode = CHAT_NO_INPUT;
                        window_textinput(0);
                        window_mousemode(WINDOW_CURSOR_DISABLED);
                ...
                return 0; /* Existing UTF-8 cursor/selection editor handles it. */
        }
        if(key == WINDOW_KEY_ESCAPE) {
                aoschat_spectator_menu = !aoschat_spectator_menu;
                if(!aoschat_spectator_menu) aoschat_playerlist_open = 0;
                window_mousemode(aoschat_spectator_menu ? WINDOW_CURSOR_ENABLED : WINDOW_CURSOR_DISABLED);
                return 1;
        }
        /* Block every gameplay/team/tool shortcut while this app owns spectator mode. */
        if(key == WINDOW_KEY_CHANGETEAM || key == WINDOW_KEY_CHANGEWEAPON
           || (key >= WINDOW_KEY_TOOL1 && key <= WINDOW_KEY_TOOL4)) return 1;
        return 0;
}

static int aoschat_spectator_mouseclick(double x, double y, int button, int action, int mods) {
        (void)mods;
        /* Consume all gameplay mouse buttons; only explicit AoS Chat controls
           below may change spectator state. */
        if(button != WINDOW_MOUSE_LMB || action != WINDOW_PRESS) return 1;
        if(aoschat_hit(aoschat_spec_menu_rect, x, y)) {
                aoschat_spectator_menu = !aoschat_spectator_menu;
                window_mousemode(aoschat_spectator_menu ? WINDOW_CURSOR_ENABLED : WINDOW_CURSOR_DISABLED);
                return 1;
        }
        if(aoschat_hit(aoschat_spec_leave_rect, x, y)) {
                cameracontroller_bodyview_mode = 0;
                cameracontroller_reset_spectator_velocity();
                return 1;
        }
        if(aoschat_spectator_menu && aoschat_hit(aoschat_spec_chat_rect, x, y)) {
                chat_input_mode = CHAT_ALL_INPUT;
                chat[0][0][0] = 0;
                chat_cursor = 0;
                chat_sel_anchor = -1;
        ...
        if(aoschat_spectator_menu && aoschat_hit(aoschat_spec_fullchat_rect, x, y)) {
                aoschat_spectator_active = 0;
                aoschat_spectator_menu = 0;
                aoschat_playerlist_open = 0;
                cameracontroller_bodyview_mode = 0;
                hud_change(&hud_aoschat);
                return 1;
        }
        if(aoschat_spectator_menu && aoschat_hit(aoschat_spec_players_rect, x, y)) {
                aoschat_playerlist_open = !aoschat_playerlist_open;
                return 1;
        }
        if(aoschat_playerlist_open) {
                for(int k = 0; k < aoschat_spec_row_count; k++) {
                        if(aoschat_hit(aoschat_spec_rows[k], x, y)) {
                                int id = aoschat_spec_row_ids[k];
                                if(aoschat_selected_player == id) {
                                        cameracontroller_bodyview_player = id;
                                        cameracontroller_bodyview_mode = 1;
                                        aoschat_playerlist_open = 0;
                                        aoschat_spectator_menu = 0;
                                        window_mousemode(WINDOW_CURSOR_DISABLED);
                                } else aoschat_selected_player = id;
                                return 1;
                        }
                }
        }
...
void hud_common_sidebar(mu_Context* ctx, float scalex, float scaley) {
        ...
        /* AoS Chat's landing screen is intentionally just the server browser.
           Keep only essential app actions; all game-client feature pages are
           unreachable. */
        if(hud_active == &hud_serverlist) {
                mu_text_accent_color(ctx, 1.0F);
                mu_text(ctx, "AoS Chat");
                mu_text_color_default(ctx);
                if(mu_button(ctx, "Settings")) {
                        aoschat_settings_return_connected = 0;
                        hud_change(&hud_settings);
                }
                mu_text_color(ctx, 255, 60, 60);
                if(mu_button(ctx, "Exit")) exit(0);
                mu_text_color_default(ctx);
                mu_end_panel(ctx);
                return;
        }
...
static void hud_common_nav(mu_Context* ctx, mu_Rect* frame, float scalex, float scaley) {
        ...
        if(hud_active == &hud_serverlist) {
                ...
                if(network_connected) {
                        ...
                        sprintf(total_str, "AoS Chat %s %s", AOSCHAT_VERSION, BS_VER_INFO);
                }
        }
...
static int hud_serverlist_name_button(mu_Context* ctx, const char* name, int row_index) {
        int pressed = mu_button_ex(ctx, "", 0, MU_OPT_NOFRAME);
        mu_Rect r = ctx->last_rect;
        int text_w = ctx->text_width(ctx->style->font, name, 0);
        int text_h = ctx->text_height(ctx->style->font);
        int available = max(1, r.w - ctx->style->padding * 2);
        int y = r.y + (r.h - text_h) / 2;
        mu_push_clip_rect(ctx, r);
        if(text_w <= available) {
                mu_draw_text(ctx, ctx->style->font, name, -1,
                             mu_vec2(r.x + ctx->style->padding, y),
                             ctx->style->colors[MU_COLOR_TEXT]);
        } else {
                /* Ping-pong between the beginning and end of the name. Unlike
                   a looping marquee this never leaves the Name column blank
                   between copies, so the second column remains continuously
                   visible even on extremely narrow screens. */
                int overflow = max(1, text_w - available);
                float travel = (float)overflow;
                float phase = fmodf((float)window_time() * 22.0F + row_index * 7.0F,
                                    travel * 2.0F);
                float offset = phase <= travel ? phase : travel * 2.0F - phase;
                int x = r.x + ctx->style->padding - (int)offset;
                mu_draw_text(ctx, ctx->style->font, name, -1, mu_vec2(x, y),
                             ctx->style->colors[MU_COLOR_TEXT]);
        }
...
static void hud_serverlist_render(mu_Context* ctx, float scalex, float scaley) {
        ...
        int portrait = settings.window_height > settings.window_width;
        /* Portrait uses touch-drag/wheel scrolling, so a reserved scrollbar
           gutter only wastes scarce width. A zero-width scrollbar still keeps
           the panel as microui's scroll target but lets headers/buttons reach
           the actual right edge. main.c restores the normal size every frame,
           so landscape retains its visible scrollbar. */
        if(portrait) ctx->style->scrollbar_size = 0;
        mu_Rect frame = mu_rect(0, 0, settings.window_width, settings.window_height);

        if(mu_begin_window_ex(ctx, "Main", frame, MU_OPT_NOFRAME | MU_OPT_NOTITLE | MU_OPT_NORESIZE | MU_OPT_NOHSCROLL)) {
                mu_Container* cnt = mu_get_current_container(ctx);
                cnt->rect = frame;

                /* Phone-first layout: the server list owns the full window.
                   News and the old permanent navigation column are intentionally
                   omitted so portrait screens do not need horizontal scrolling. */
                mu_layout_row(ctx, 1, (int[]) {-1}, -1);
                mu_begin_panel_ex(ctx, "Content", MU_OPT_NOHSCROLL);
                mu_Container* serverlist_content_panel = mu_get_current_container(ctx);
                ...
                int content_w = mu_get_current_container(ctx)->body.w;
                int spacing = ctx->style->spacing;
                int submit_address = 0;
                if(portrait) {
                        int input_w = max(40, content_w - c - spacing);
                        mu_layout_row(ctx, 2, (int[]) {input_w, c}, 0);
                        submit_address = hud_textbox(ctx, serverlist_input, sizeof(serverlist_input), 0) & MU_RES_SUBMIT;
                        if(mu_button_ex(ctx, "Join", 16, MU_OPT_ALIGNRIGHT))
                                server_c(serverlist_input, NULL);
                        mu_layout_row(ctx, 2, (int[]) {content_w / 2 - spacing, -1}, 0);
                        if(mu_button_ex(ctx, "Local", 16, MU_OPT_ALIGNCENTER))
                                server_c("aos://16777343:32887", NULL);
                        if(mu_button_ex(ctx, "Refresh", 17, MU_OPT_ALIGNCENTER) && !request_serverlist)
                                hud_serverlist_init();
                } else {
                        int input_w = max(40, content_w - a - b - c - spacing * 3);
                        mu_layout_row(ctx, 4, (int[]) {input_w, c, b, a}, 0);
                        submit_address = hud_textbox(ctx, serverlist_input, sizeof(serverlist_input), 0) & MU_RES_SUBMIT;
                        if(mu_button_ex(ctx, "Join", 16, MU_OPT_ALIGNRIGHT))
                                server_c(serverlist_input, NULL);
                ...
                if(submit_address) server_c(serverlist_input, NULL);

                int row_h = ctx->style->size.y + ctx->style->padding * 2;
                /* Leave exactly one final control row for Settings so it sits
                   against the content panel's bottom padding, not floating above it. */
                int server_h = settings.window_height - (portrait ? 4 : 3) * row_h - ctx->style->padding * 4;
                if(server_h < row_h * 4) server_h = row_h * 4;
                mu_layout_row(ctx, 1, (int[]) {-1}, server_h);
                mu_begin_panel_ex(ctx, "Servers", MU_OPT_NOHSCROLL);
                mu_Container* serverlist_servers_panel = mu_get_current_container(ctx);
                mu_layout_row(ctx, 1, (int[]) {-1}, 0);
                ...
                if(col0_w < players_w)
                        col0_w = players_w;
                if(portrait) {
                        /* Guarantee the Name column a majority of the width even
                           when high-DPI UI scaling makes the Players label wide. */
                        int min_name_w = max(80, (int)(width * 0.55F));
                        col0_w = min(col0_w, width - min_name_w - ctx->style->spacing);
                        col0_w = max(40, col0_w);
                }

                /* A clearly visible right gutter keeps the Name header/card
                   border separate from the outer server-panel border. */
                int table_right_pad = max((int)(14 * hud_ui_scale()), ctx->style->padding * 2);

                if(portrait) {
                        /* Phone layout: Players + Name. Long names marquee inside
                           the Name cell instead of creating a horizontal scrollbar. */
                        mu_layout_row(ctx, 2,
                                      (int[]) {col0_w, width - col0_w - ctx->style->spacing - table_right_pad}, 0);
                        ...
                        /* Desktop also has only Players + Name. The country flag
                           is rendered inside the Name column; Map, Mode and Ping
                        ...
                        mu_layout_row(ctx, 3,
                                      (int[]) {col0_w, flag_width,
                                               width - col0_w - flag_width - ctx->style->spacing * 2 - table_right_pad}, 0);
                }
                ...
                if(server_count > 0) {
                        ...
                        for(int k = 0; k < server_count; k++) {
                                ...
                                if(mu_button_ex(ctx, total_str, 0, MU_OPT_NOFRAME | MU_OPT_ALIGNCENTER))
                                        tapped = true;
                                if(portrait) {
                                        if(hud_serverlist_name_button(ctx, serverlist[k].name, k))
                                                tapped = true;
                                } else {
                ...
                mu_end_panel(ctx);
                if(portrait) {
                        serverlist_servers_panel->scroll.x = 0;
                        serverlist_servers_panel->content_size.x = serverlist_servers_panel->body.w;
                }

                /* Bottom app action replaces the wasteful permanent sidebar. */
                mu_layout_row(ctx, 1, (int[]) {-1}, 0);
                if(mu_button(ctx, "Settings")) {
                        aoschat_settings_return_connected = 0;
                        hud_change(&hud_settings);
                ...
                mu_end_panel(ctx);
                if(portrait) {
                        serverlist_content_panel->scroll.x = 0;
                        serverlist_content_panel->content_size.x = serverlist_content_panel->body.w;
                        cnt->scroll.x = 0;
                        cnt->content_size.x = cnt->body.w;
                }
...
static int aoschat_setting_allowed(const char* name) {
#ifdef __ANDROID__
        if(!strcmp(name, "Lock to portrait mode")) return 1;
#endif
        return !strcmp(name, "Name")
                || !strcmp(name, "Mouse sensitivity")
                || !strcmp(name, "Camera FOV")
                || !strcmp(name, "UI Accent: Red")
                || !strcmp(name, "UI Accent: Green")
                || !strcmp(name, "UI Accent: Blue");
}
...
static void hud_settings_render(mu_Context* ctx, float scalex, float scaley) {
        ...
        /* AoS Chat intentionally exposes only identity, spectator camera and
           accent controls. All gameplay/graphics/post-processing pages remain
           compiled for engine compatibility but are unreachable. */
        if(mu_begin_window_ex(ctx, "AoS Chat Settings", frame,
                             MU_OPT_NOFRAME | MU_OPT_NOTITLE | MU_OPT_NORESIZE)) {
                mu_Container* root = mu_get_current_container(ctx);
                root->rect = frame;
                int back_w = (int)(100 * hud_ui_scale());
                mu_layout_row(ctx, 2, (int[]){back_w, -1}, 0);
                if(mu_button(ctx, "Back"))
                        hud_change(aoschat_settings_return_connected ? &hud_aoschat : &hud_serverlist);
                mu_text(ctx, "AoS Chat Settings");
                mu_layout_row(ctx, 1, (int[]){-1}, -1);
                mu_begin_panel(ctx, "Settings");
                int width = mu_get_current_container(ctx)->body.w;
                for(int k = 0; k < list_size(&config_settings); k++) {
                        struct config_setting* a = list_get(&config_settings, k);
                        if(aoschat_setting_allowed(a->name))
        ...
        /* Legacy settings renderer retained below but deliberately unreachable;
           keeping it avoids destabilizing engine configuration internals. */
