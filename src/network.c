...
void read_PacketStateData(void* data, int len) {
        ...
        if(demo_is_playing()) {
                ...
                /* AoS Chat never exposes team selection or gameplay. Join as a
                   spectator immediately after state/map initialization, then
                   keep the dedicated fullscreen chat HUD in front. */
                local_player_newteam = TEAM_SPECTATOR;
                camera_mode = CAMERAMODE_SPECTATOR;
                screen_current = SCREEN_NONE;
                cameracontroller_reset_spectator_velocity();
                ...
                strncpy(login.name, settings.name, sizeof(login.name) - 1);
                network_send(PACKET_EXISTINGPLAYER_ID, &login,
                             sizeof(login) - sizeof(login.name) + strlen(login.name) + 1);
...
void read_PacketVersionGet(void* data, int len) {
        ...
        ver.client = 'K';
        ver.major = AOSCHAT_MAJOR;
        ver.minor = AOSCHAT_MINOR;
        ver.revision = AOSCHAT_PATCH;
#ifndef OPENGL_ES
#ifdef OS_WINDOWS
        char* os = "AoS Chat (Windows) " BS_VER_INFO;
#endif
#ifdef OS_LINUX
        char* os = "AoS Chat (Linux) " BS_VER_INFO;
#endif
#ifdef OS_APPLE
        char* os = "AoS Chat (Apple) " BS_VER_INFO;
#endif
#ifdef OS_HAIKU
        char* os = "AoS Chat (Haiku) " BS_VER_INFO;
#endif
#else
#if defined(OS_IOS)
        char* os = "AoS Chat (iOS) " BS_VER_INFO;
#elif defined(USE_TOUCH)
        char* os = "AoS Chat (Android) " BS_VER_INFO;
#else
        char* os = "AoS Chat (Embedded) " BS_VER_INFO;
#endif
#endif
